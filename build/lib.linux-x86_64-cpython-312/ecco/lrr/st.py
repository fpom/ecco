...
sign2sign = {True: True, "+": True, False: False, "-": False, None: None, "*": None}

sign2char = {True: "+", False: "-", None: "*"}

sign2neg = {True: False, False: True, None: None}
...
class Rule(Record):
    ...
    def normalise(self):
        missing = tuple(
            s for s in self.left if s not in self.right and ~s not in self.right
        )
        if missing:
    ...
    def elementarise(self):
        ...
        if not diff:
            ...
            for n, t in enumerate(toadd, start=1):
                yield rule.__class__(
                    rule.left + tuple(sorted(t)),
                    rule.right,
                    parent=self,
                    label=self.label,
                    num=n,
    ...
    def text(self):
        return "%s >> %s" % (
            ", ".join(s.name + ("+" if s.sign else "-") for s in self.left),
            ", ".join(s.name + ("+" if s.sign else "-") for s in self.right),
        )
    ...
    def inline(self, other):
        ...
        if cond & right and not any(~c in reach for c in cond):
            ...
            yield self.__class__(
                self.left + [c for c in missing if c not in reach],
            ...
            for m in missing:
                yield self.__class__(
                    self.left + [~m], self.right, parent=self, label=self.label
                )
        else:
            yield self
...
class Sort(Record):
    ...
    def __lt__(self, other):
        if self.state.sign is None and other.state.sign is None:
            ...
            return (self.state.name, self.state.sign) < (
                other.state.name,
                other.state.sign,
            )
...
class Spec(Record):
    ...
    def __str__(self):
        return "\n".join(
            [
                "\n".join(
                    "# %s (%s): %s" % (sort.state, sort.kind, sort.description)
                    for sort in self.meta
                ),
                "\n".join(str(rule) for rule in self.constraints)
                or "## no constraints",
...
class Parser(object):
    def __init__(self, path, source=None):
        ...
        src = (source or open(path, encoding="utf-8", errors="replace").read()) + "\n"
    ...
    def vdecl(self, st):
        ...
        """
        self._meta.extend(
            Sort(state, st.name, description.split("#", 1)[0].strip())
