...
class Palette(object):
    palettes = {
        ...
        # pride flags
        "gay pride": ["#FF0000", "#FFA500", "#FFFF00", "#008000", "#0000FF",
                      "#4B0082"],
        ...
        'Belize': ['#CE1126', '#003F87', '#FFFFFF', '#289400', '#FFD83C',
                   '#9DD7FF'],
        ...
        'Dominica': ['#006B3F', '#FCD116', '#000000', '#FFFFFF', '#D41C30',
                     '#9461C9'],
        'Dominican Republic': ['#002D62', '#FFFFFF', '#CE1126', '#EAC102',
                               '#008337'],
        'East Timor': ['#000000', '#FFFFFF', '#FFC726', '#DC241F'],
        'Ecuador': ['#FFDD00', '#034EA2', '#ED1C24', '#452C25', '#B87510',
                    '#086F35'],
        'Egypt': ['#CE1126', '#FFFFFF', '#C09300', '#000000'],
        'El Salvador': ['#0F47AF', '#FFFFFF', '#FFCC00', '#1E5B19', '#E60000'],
        'Equatorial Guinea': ['#000000', '#0073CE', '#3E9A00',
                              '#FFFFFF', '#E32118', '#FFD700'],
        ...
        'Fiji': ['#002868', '#68BFE5', '#FFFFFF', '#CE1126', '#FFD100',
                 '#00A651'],
        ...
        'Guatemala': ['#4997D0', '#FFFFFF', '#448127', '#F9F0AA', '#6C301E',
                      '#B2B6BA'],
        ...
        'Nicaragua': ['#0067C6', '#FFFFFF', '#C9A504', '#6FD8F3', '#97C924',
                      '#FF0000'],
        ...
        'Paraguay': ['#D52B1E', '#FFFFFF', '#0038A8', '#000000', '#009A3A',
                     '#FEDF00'],
        ...
        'South Africa': ['#000000', '#FFB612', '#007A4D', '#FFFFFF',
                         '#DE3831', '#002395'],
        'South Korea': ['#000000', '#FFFFFF', '#CD2E3A', '#0047A0'],
        'South Sudan': ['#0F47AF', '#FCDD09', '#000000', '#FFFFFF',
                        '#DA121A', '#078930'],
        'Spain': ['#AA151B', '#F1BF00', '#0039F0', '#CCCCCC', '#ED72AA',
                  '#058E6E'],
    ...
    def __repr__(self):
        return (f"{self.__class__.__name__}"
                f"({self.name!r}, {self.mode!r}, {self.sort})")
    ...
    def _ipython_display_(self, width=400):
        ...
        display(ipw.VBox([ipw.HTML(f"<b>{self.name}</b>"
                                   f" ({len(self.colors)} {c})"),
                          ipw.HTML("".join(f"<span style="
                                           f"{s.format(col=col)!r}></span>"
...
class NodeShowDesc(_Desc):
    modes = {"all": {True: [True, True, 1.0],
                     False: [True, True, 1.0]},
             "dim": {True: [True, True, 0.2],
                     False: [True, True, 1.0]},
             "hide": {True: [True, False, 1.0],
                      False: [True, True, 1.0]},
             "drop": {True: [False, True, 1.0],
                      False: [True, True, 1.0]}}
        ...
        if layout == "grid":
            ...
            if layout.startswith("xx-"):
                ...
                if layout.startswith("nx-"):
                    ...
                    pos = {k[8:] if k.startswith("cluster_")
                           and k[8:] in graph.groups
...
class Graph(object):
    ...
    Node labels options:
     ...
     - `node_label_align`: both horizontal and vertical placement of labels
       ...
       because it specifies twice horizontal alignement. To avoid this, start
    ...
    Edges options:
     ...
     - `edges_draw_color`, `edges_draw_palette`, `edges_draw_style`,
       ...
       that information is taken from table `edges` is column names are used)
    ...
    Additional properties gather other properties, so assigned them will
    ...
    organised as nested lists and dicts:
     ...
     - atomic elements are:
        - the name of an option listed above (except the additional ones at
          the end) to build a widget allowing to tune this option
    ...
    mandatory to callable method `update` to actually update the display.
    `Graph` instance also supports item assignement to update one option for
    just one node or edge, see `__setitem__` documentation.
    """
    ...
    nodes_fill_color = TableColorDesc("nodes", "_ns", "background-color",
                                      "size")
    ...
    nodes_draw_width = TableNumberDesc("nodes", "_ns", "border-width",
                                       0., 0., 10.)
    nodes_draw_style = TableStyleDesc("nodes", "_ns", "border-style", "|")
    nodes_show_display = TableDisplayDesc("nodes", "_ns", "display",
                                          "element", "display")
    nodes_show_visibility = TableVisibilityDesc("nodes", "_ns", "visibility",
                                                "visible", "visibility")
    nodes_show_opacity = TableNumberDesc("nodes", "_ns", "opacity",
                                         1.0, 0.0, 1.0)
    ...
    nodes_label_size = TableNumberDesc("nodes", "_ns", "font-size",
                                       8., 0., 40.)
    nodes_label_valign = TableValignDesc("nodes", "_ns",
                                         "text-valign", "center")
    nodes_label_halign = TableHalignDesc("nodes", "_ns",
                                         "text-halign", "center")
    ...
    nodes_label_outline_opacity = TableNumberDesc("nodes", "_ns",
                                                  ...
                                                  0., 0., 1.)
    nodes_label_outline_width = TableNumberDesc("nodes", "_ns",
                                                "text-outline-width",
                                                0., 0., 10.)
    # edges
    ...
    edges_label_size = TableNumberDesc("edges", "_es", "font-size",
                                       6., 0., 40.)
    edges_label_angle = TableAngleDesc("edges", "_es",
    ...
    edges_label_outline_opacity = TableNumberDesc("edges", "_es",
                                                  "text-outline-opacity",
                                                  .75, 0., 1.)
    edges_label_outline_width = TableNumberDesc("edges", "_es",
                                                "text-outline-width",
                                                3., 0., 10.)
    edges_show_display = TableDisplayDesc("edges", "_es", "display",
                                          "element", "display")
    edges_show_visibility = TableVisibilityDesc("edges", "_es", "visibility",
                                                "visible", "visibility")
    edges_show_opacity = TableNumberDesc("edges", "_es", "opacity",
                                         1.0, 0.0, 1.0)
    # tips
    edges_tip_scale = TableNumberDesc("edges", "_es", "arrow-scale",
                                      .6, 0., 5.)
    edges_target_tip = TableTipDesc("edges", "_es", "target-arrow-shape", ">")
    edges_target_color = TableColorDesc("edges", "_es",
                                        "target-arrow-color", "black")
    edges_target_palette = PaletteDesc("black")
    edges_source_tip = TableTipDesc("edges", "_es", "source-arrow-shape", "-")
    edges_source_color = TableColorDesc("edges", "_es",
    ...
    nodes_label_align = TableAlignDesc("nodes_label_halign",
                                       "nodes_label_valign")
    edges_tip_color = GroupDesc("edges_target_color", "edges_source_color")
    edges_tip_palette = GroupDesc("edges_target_palette",
                                  "edges_source_palette")
    ...
    def __init__(self, nodes, edges, **args):
        ...
        self._inspect_nodes_cols = args.pop("inspect_nodes",
                                            self.nodes.columns)
        self._inspect_edges_cols = args.pop("inspect_edges",
                                            self.edges.columns)
        ...
        if hierarchy:
            self.nodes.sort_values(by=hierarchy, inplace=True,
                                   na_position="first")
    ...
    def legend(self, option="nodes_fill", values=None,
        ...
        if values is None:
            ...
            if col is None:
                raise ValueError("unknown color source,"
    ...
    def _unselect_node(self, node):
        node.classes = " ".join(c for c in node.classes.split()
                                if c != "selected")
    ...
    def _make_ui_widget_nodes_show(self, name, opt, columns=[]):
        drop_mode = ipw.Dropdown(options=[(str(m), m)
                                          for m in NodeShowDesc.modes],
                                 value="all", description="show")
        columns = columns or list(self.nodes.columns)
        drop_col = ipw.Dropdown(options=[(str(c), c) for c in columns],
                                value=columns[0], description="column",
                                disabled=True)
        invert_check = ipw.Checkbox(value=False, description="invert",
                                    disabled=True)
        dim_entry = ipw.BoundedFloatText(value=0.2,
        ...
        def on_mode(event):
            ...
            if choice == "all":
                ...
                self.nodes_show = (choice, drop_col.value,
                                   invert_check.value, dim_entry.value)
            self.update()

        drop_mode.observe(on_mode, names="value")

        def on_col(event):
            self.nodes_show = (drop_mode.value, event["new"],
                               invert_check.value, dim_entry.value)
            self.update()
        ...
        def on_invert(event):
            self.nodes_show = (drop_mode.value, drop_col.value,
                               event["new"], dim_entry.value)
            self.update()
    ...
    def _make_ui_widget_LayoutDesc(self, name, opt):
        ...
        options = list(sorted((opt.values
                               | set(f"xx-{x}" for x in self.layout_extra))
                              - {"preset"}))
        libs = {"gv": "GraphViz",
                "nx": "NetworkX",
                "ig": "igraph",
    ...
    def _make_ui_widget_PaletteDesc(self, name, opt):
        ...
        drop_name = ipw.Dropdown(options=list(sorted(Palette.palettes,
                                                     key=str.lower)),
                                 value=palette.name,
                                 description="palette")
    ...
    def _make_ui_widget_TableStrDesc(self, name, opt):
        table = getattr(self, opt.table)
        default = opt.default
        drop = ipw.Dropdown(options=["(default)"]
                            + list(sorted(table.columns, key=str.lower)),
                            value="(default)",
                            description=name.split("_")[-1])
    ...
    def _make_ui_widget_TableAngleDesc(self, name, opt):
        ...
        options.extend(col for col in table.columns
                       if is_numeric_dtype(table[col]))
        drop = ipw.Dropdown(options=options,
                            value="(default)",
                            description=name.split("_")[-1])
    ...
    def _export_image(self, nodes, edges, fmt):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                subprocess.check_output(["lualatex",
