...
class Model(BaseModel):
    ...
    def latex(self, out, sty=None, names=False, cols=False):
        ...
        if names:
            width = max(len(r.text())
                        + (len("["+r.label.strip()+"] ") if r.label else 0)
    ...
    def gal(self, compact=False, permissive=False, showlog=True, init={}):
        ...
        with log(head="<b>saving</b>",
            ...
            for sort in self.spec.meta:
                ...
                out.write(f"    // {sort.state}: {sort.description}"
                          f" ({sort.kind})\n"
    ...
    def zeno(self, witness: int = 10) -> Optional[list]:
        ...
        with log(head="<b>checking for Zeno executions</b>",
                 tail="{step}",
                 done_head="<b>Zeno:</b>",
                 done_tail=('<span style="color:#080; font-weight:bold;">'
                            'no</span>'),
            ...
            if hull:
                log.done_tail = ('<span style="color:#800; font-weight:bold;">'
                                 'YES</span>')
            else:
                log.finish()
                return
    ...
    def CTL(self, formula, compact=False, options=[], debug=False, init={}):
        ...
        with tempfile.NamedTemporaryFile(mode="w+", suffix=f".ctl") as tmp:
            ...
            try:
                ...
                log.err(f"<tt>its-ctl</tt> exited with status code"
                        f" {err.returncode}")
                out = err.output
                debug = True
        if debug:
    ...
    def LTL(self,
        ...
        try:
            ...
            log.err(f"<tt>its-ltl</tt> exited with status code"
                    f" {err.returncode}")
            out = err.output
            debug = True
        if debug:
    ...
    def ecohyper(self, constraints: bool = True, **opt):
        ...
        edges = []
        if constraints:
            rules = list(itertools.chain(self.spec.constraints,
                                         self.spec.rules))
        else:
            rules = list(self.spec.rules)
        for state, desc in sorted((s.state, s.description)
                                  for s in self.spec.meta):
        ...
        for rule in rules:
            ...
            for var in set(s.name for s in itertools.chain(rule.left,
                                                           rule.right)):
                v0 = State(var, False)
                v1 = State(var, True)
                if v1 in rule.left:
    ...
    def _petri_actions(self, transform=0):
        for prio, actions in enumerate([self.spec.rules,
    ...
    def pep(self, stream, epn=True):
        ...
        if epn:
            ...
            self._pep_ppn(stream, states,
                          (n for r in rules for n in r.elementarise()))
    ...
    def unfold(self, petri=False, compress=True):
        ...
        subprocess.check_output(["ecofolder", pre_path, "-m", mci_path]
                                ...
                                stderr=subprocess.STDOUT,
                                encoding="ascii", errors="replace")
        subprocess.check_output(["mci2csv", mci_path]
                                + (["-c"] if compress else []),
                                stderr=subprocess.STDOUT,
                                encoding="ascii", errors="replace")
        ...
        if compress:
            pdopts = {"converters": {"cutoff": cutoff,
                                     "tokens": tokens}}
        else:
            pdopts = {"converters": {"cutoff": cutoff},
                      "dtype": {"tokens": "Int64"}}
        nodes = pd.read_csv(nodes_path, **pdopts)\
                .rename(columns={"id": "node"})
        if compress:
            c2g = {g.rsplit(",", 1)[-1]: g for g in nodes["node"]}
            pdopts = {"converters": {"src": c2g.get,
                                     "dst": c2g.get}}
        else:
            pdopts = {}
        ...
        if petri:
            ...
            conf = {frozenset([a, b])
                    ...
                    if c not in events
                    and a != b}
            edges = pd.DataFrame([("_", r, "causality") for r in root]
...
class ComponentGraph(object):
    def __init__(self,
        ...
        if compact and not self.model.spec.constraints:
            log.warn("model has no constraints,"
                     " setting <code>compact=False</code>")
            compact = False
        if lts is None:
    ...
    def from_model(cls, model, compact=False, init="", split=True):
        ...
        if split:
            cg.components = tuple(c for c
                                  in c_all.topo_split(split_entries=False,
                                                      split_exits=False)
                                  if c is not None)
        else:
            cg.components = (c_all,)
    ...
    def edges(self):
        ...
        if self.model.spec.labels:
            ...
            def lbl(rules):
                labels = set()
                for r in rules:
                    labels.update(lbl.strip()
                                  for lbl in (spec_labels.get(r)
                                              or "").split(",")
                                  if lbl.strip())
                return hset(labels)
    ...
    def _get_args(self,
        ...
        if not min_props <= len(properties) <= _max_props:
            if min_props == max_props == 0:
                ...
                raise TypeError(f"expected {min_props} to {_max_props}"
        ...
        if not min_compo <= len(components) <= _max_compo:
            if min_compo == max_compo == 0:
                ...
                raise TypeError(f"expected {min_compo} to {_max_compo}"
    ...
    def check(self, *args, **aliased):
        ...
        for prop, alias in props.items():
            ...
            for c in compo:
                ret[c.num][alias] = setrel(c.check(prop,
                                                   checker(c.states),
                                                   alias)).name
    ...
    def scc_split(self, *args, limit=256):
        ...
        while todo:
            ...
            todo = {c.num for c in g.components
            ...
            if limit and len(g) > _limit:
                answer = input(f"We have created a graph with {len(g)} nodes,"
    ...
    def explicit(self, *args, limit=256):
        ...
        _, compos = self._get_args(args, max_props=0)
        if limit > 0:
            size = (sum((len(c) for c in compos), 0)
                    + len(self.components)
                    - len(compos))
            if size > limit:
    ...
    def search(self, *args, prune=False, stutter=False, col="search", **props):
        ...
        if len(seq) <= 1:
            log.warn(f"only {len(seq)} non-empty properties,"
                     f" no path can be found")
            return cg
        ...
        nodes = cg.g.vs["node"]
        edges = {tuple((nodes[s], nodes[t]) for s, t in p
                       if s is not None and t is not None)
                 for p in paths if len(p) == minlen}
